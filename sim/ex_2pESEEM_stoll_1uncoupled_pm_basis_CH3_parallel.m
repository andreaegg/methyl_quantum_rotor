...
function [t,signal] = ex_2pESEEM_stoll_1uncoupled_pm_basis_CH3_parallel(Sys,Exp,Opt)
...
if Sys.methyl == 1
    if (length(find(Sys.Itype == "1H")) >= 3 && isequal(find(Sys.Itype == "1H"),[1 2 3]))
        ...
        i3xi1x = sop(spinvec,'exex');

        i1yi2y = sop(spinvec,'eyye');
        i2yi3y = sop(spinvec,'eeyy');
        i3yi1y = sop(spinvec,'eyey');
        
        i1zi2z = sop(spinvec,'ezze');
        i2zi3z = sop(spinvec,'eezz');
        i3zi1z = sop(spinvec,'ezez');
        rmqr = -(2*Sys.vt/3)*(i1xi2x + i1yi2y + i1zi2z + i2xi3x + i2yi3y + i2zi3z + i3xi1x + i3yi1y + i3zi1z);
    else
