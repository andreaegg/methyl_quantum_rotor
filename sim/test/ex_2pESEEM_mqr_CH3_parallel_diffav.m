...
function [t,signal] = ex_2pESEEM_mqr_CH3_parallel_diffav(Sys,Exp,Opt)
...
% Rotate molecule to z-axis position
...
alpha  = atan(uv_dd(1,1)/uv_dd(1,2));
beta   = atan((sin(alpha)*uv_dd(1,1) + cos(alpha)*uv_dd(1,2))/uv_dd(1,3));
B0     = [0 0 1];
uv_new = (Rx(beta)*Rz(alpha)*(uv_dd'))';
if uv_new(1,3) < 0
    beta = beta + pi;
    uv_new = (Rx(beta)*Rz(alpha)*(uv_dd'))';
end
...
parfor ori = 1:nori
    ...
    if Sys.Inum > 0
        ...
        cvec = vecs(:,ori)';
        for k = 1:Sys.Inum
            % Rotation of molecular vectors
            if k == 1
                theta = acos(cvec(3));
                phi   = asin(cvec(1)/sin(theta));
                if theta == 0
                   phi = 0;
                end
            end
            uv = (Rz(phi)*Rx(theta)*(uv_new(k,:)'))';
            % calculate HF coupling
            ct_dd  = sum(B0.*uv);
