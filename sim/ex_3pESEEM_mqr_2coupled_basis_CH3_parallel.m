function [t,signal] = ex_3pESEEM_mqr_2coupled_basis_CH3_parallel(Sys,Exp,Opt)
...
%       .Icoord     coordinates of nuclear spin(s) of methyl group (rows = nucleus, columns = [x y z])
%                    --> if MQR should be considered protons must be written in positions 1,2,3
%       .HFiso       isotropic HF contribution [MHz] (rows = Aiso , column = nucleus)
%                    default = 0
%       .methyl      1 if 1 methyl group  is in close proximity of e-spin
%                    2 if 2 methyl groups is in close proximity of e-spin
%                    default = 0
...
if Sys.methyl > 0
    if ~isfield(Sys,'vt')
        Sys.vt = 0.2*ones(length(Sys.methyl),1);
    else
        validateattributes(Sys.vt,{'numeric'},{'nonnegative','numel',Sys.methyl})
    end
end
...
for ori = 1:nori
    ...
    for p = 1:npoints
        ...
        for q = 1:length(Exp.addphase)
            ...
            sig_tmp = (udt^(p-1))*sig_tmp*(udtr^(p-1));   % +dt (T)
