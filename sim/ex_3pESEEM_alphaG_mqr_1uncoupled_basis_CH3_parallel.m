function [t,signal,ham] = ex_3pESEEM_alphaG_mqr_1uncoupled_basis_CH3_parallel(Sys,Exp,Opt)
...
switch multiplenuc
    ...
    case 1
        ...
        sa   = sop(spinvec,strcat('a',stringvec));
        for k = 1:Sys.Inum
            ...
            stringvec      = repmat('e',1,Sys.Inum);
            stringvec(k) = 'z';
            saiz{k} = sop(spinvec,strcat('a',stringvec));

            stringvec    = repmat('e',1,Sys.Inum);
            stringvec(k) = 'x';
            saix{k} = sop(spinvec,strcat('a',stringvec));
        end
        n    = size(sx,1);
        sm   = sx - 1i*sy;
        sig0 = -sz;
...
for ori = 1:nori
    ...
    if Sys.Inum > 0
        ...
        for k = 1:Sys.Inum
            ...
            ham = ham + wI(k)*iz{k} + a*saiz{k};
            if Sys.methyl == 1
                helpvec = repmat([1 2 3],1,2);
                if k <= 3
                    sub  = sub + a*saiz{k};
                    addhamr2 = addhamr2 + a*saiz{helpvec(k+2)};
                    addhamr3 = addhamr3 + a*saiz{helpvec(k+1)};
                end
            end
        end
        if Sys.methyl == 1
