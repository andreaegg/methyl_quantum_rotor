...
function [t,signal] = ex_2pESEEM_mqr_1uncoupled_pm_ABterms_basis_CH3_parallel(Sys,Exp,Opt)
...
for ori = 1:nori
    ...
    if Sys.Inum > 0
        ...
        if Sys.methyl == 1
            ...
            hamalpha(1:8,1:8)    = ham(1:8,1:8);
            hamalpha(1:8,9:16)   = ham(1:8,17:24);
            hamalpha(1:8,17:24)  = ham(1:8,33:40);
            hamalpha(9:16,1:8)   = ham(17:24,1:8);
            hamalpha(9:16,9:16)  = ham(17:24,17:24);
            hamalpha(9:16,17:24) = ham(17:24,33:40);
            hamalpha(17:24,1:8)  = ham(33:40,1:8);
            hamalpha(17:24,9:16) = ham(33:40,17:24);
            hamalpha(17:24,17:24)= ham(33:40,33:40);
            
            
            hambeta(1:8,1:8)    = ham(9:16,9:16);
            hambeta(1:8,9:16)   = ham(9:16,25:32);
            hambeta(1:8,17:24)  = ham(9:16,41:48);
            hambeta(9:16,1:8)   = ham(25:32,9:16);
            hambeta(9:16,9:16)  = ham(25:32,25:32);
            hambeta(9:16,17:24) = ham(25:32,41:48);
            hambeta(17:24,1:8)  = ham(41:48,9:16);
            hambeta(17:24,9:16) = ham(41:48,25:32);
            hambeta(17:24,17:24)= ham(41:48,41:48);
        end
    end
    ...
    for p = 1:npoints
        Ua_tau = expm(-1i*2*pi*Ea*(Exp.tau + (p-1)*Exp.dt));
        Ub_tau = expm(-1i*2*pi*Eb*(Exp.tau + (p-1)*Exp.dt));
        sig_tmp = Ub_tau*(M')*Ua_tau*M*(Ub_tau')*(M')*(Ua_tau');
        csignal(p) = trace(M*sig_tmp)/2;
    end
