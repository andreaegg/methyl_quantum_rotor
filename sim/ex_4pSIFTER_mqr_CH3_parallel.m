...
function [t,signal] = ex_4pSIFTER_mqr_CH3_parallel(Sys,Exp,Opt)
...
parfor ori = 1:nori
    ...
    p4_pc = phasecycle(w_pi,Exp.phase(4,:),{sx,sy});        % generate pi pulse with phase cycle as cell
    
    % Generation of propagators
    [upi2_1,upi2r_1] = gen_propagators(Exp.tpi2,p1_pc);
    [upi_2,upir_2]   = gen_propagators(Exp.tpi,p2_pc);
    [upi2_3,upi2r_3] = gen_propagators(Exp.tpi2,p3_pc);
    [upi_4,upir_4]   = gen_propagators(Exp.tpi,p4_pc);
    ...
    for p = 1:npoints
        ...
        for q = 1:length(Exp.addphase)
            ...
            sig_tmp = utau1*sig_tmp*utaur1;              % tau1
            sig_tmp = upi2_3{q}*sig_tmp*upi2r_3{q};      % pi pulse
            sig_tmp = utau2*sig_tmp*utaur2;              % tau2
            sig_tmp = upi_4{q}*sig_tmp*upir_4{q};        % pi pulse
            sig_tmp = utau2*sig_tmp*utaur2;              % tau2
