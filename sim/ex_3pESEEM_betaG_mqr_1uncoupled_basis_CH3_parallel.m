function [t,signal,ham] = ex_3pESEEM_betaG_mqr_1uncoupled_basis_CH3_parallel(Sys,Exp,Opt)
...
switch multiplenuc
    ...
    case 1
        ...
        sb   = sop(spinvec,strcat('b',stringvec));
        for k = 1:Sys.Inum
            ...
            stringvec      = repmat('e',1,Sys.Inum);
            stringvec(k) = 'z';
            sbiz{k} = sop(spinvec,strcat('b',stringvec));
...
for ori = 1:nori
    ...
    if Sys.Inum > 0
        ...
        for k = 1:Sys.Inum
            ...
            ham = ham - wI(k)*iz{k} + a*sbiz{k}; 
            if Sys.methyl == 1
                helpvec = repmat([1 2 3],1,2);
                if k <= 3
                    sub  = sub + a*sbiz{k};
                    addhamr2 = addhamr2 + a*sbiz{helpvec(k+2)};
                    addhamr3 = addhamr3 + a*sbiz{helpvec(k+1)};
                end
            end
        end
        if Sys.methyl == 1
