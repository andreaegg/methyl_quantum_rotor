function [t,signal] = ex_CPDD3_mqr_CH3_parallel(Sys,Exp,Opt)
...
if ~exist('Exp','var')
    ...
    if size(Exp.phase,1) > (length(Exp.tpi2)+3*length(Exp.tpi))
        error('Number of phase cycled pulses cannot exeed number of applied pulses in pulse sequence.')
    end
    if size(Exp.addphase,1) > (length(Exp.tpi2)+3*length(Exp.tpi))
        error('Number of phase cycled pulses cannot exeed number of applied pulses in pulse sequence.')
    end
...
npoints = Exp.npoints;
t = linspace(3*Exp.tau,3*Exp.tau +3*(Exp.npoints-1)*Exp.dt,Exp.npoints);
