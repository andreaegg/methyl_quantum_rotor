function [t,signal,hambeta] = ex_3pESEEM_beta_mqr_1uncoupled_basis_CH3_parallel(Sys,Exp,Opt)
...
for ori = 1:nori
    ...
    sig0 = (M')*Ua_tau*M*(Ub_tau');
    for p = 1:npoints
        Ub_T = expm(-1i*2*pi*Eb*(Exp.T+(p-1)*Exp.dt));
        sig_tmp = Ub_T*sig0*(Ub_T');
        sig_tmp = Ub_tau*sig_tmp*(M')*(Ua_tau');
