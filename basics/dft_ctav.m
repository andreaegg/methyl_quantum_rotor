function [nu,spc,window] = dft_ctav(t,signal,options)
% function [nu,spc,window] = dft_ctav(t,signal,options)
%
...
%                               defaults to 'complex'
%           .ctav               if set to N>0 turns on cross-term averaging 
%                               using N averages of the signal where the 
%                               first 1:N points are removed, 
%                               see Doorslaer, Sierra, Schweiger, 
%                               J. Magn. Reson. 136, 152 (1999), defaults
%                               to 0/false. Returns only power spectrum.
%
...
if ~options.ctav,
    ...
    % FFT with cross-term averaging
    if options.ctav > 1 && options.ctav < length(signal),
        i_ctav = 1:options.ctav;
    else
        i_ctav = 1:length(signal);
    end;
    
    spc = 0;
    for i = i_ctav,
        spc = spc + abs(fft(signal(i:end),length(signal))).^2; % fft & sum up power spectra
    end;
    spc = sqrt(spc./i_ctav(end));
    if ~strncmpi(options.mode, 'power', 5) && ~strncmpi(options.mode, 'complex', 7)
        warning('Only power spectrum is returned by cross-termed averaged FFT.');
    end
    options.mode = 'none'; % already power spc, prevent further change below
end;
...
if n <= 2048
    ...
    window=window(1:n-1);
    window(n) = window(n-1); % correct edge-error for many points in chebwin
end
for k=1:m,
